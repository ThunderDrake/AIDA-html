 ...
 // start функции
  ...
  function scroolSpeed(num, trysy = false){
    if(window.innerWidth > 1920 && (trysy || !trysy)){
      if(trysy){
        return num * window.innerWidth / 1200;
      } else{
  ...
  function trysyGsap(){
    ...
    for(let i = 0; i < allTrysy.length; i++){
      ...
      e.to(".we-do-email-item_wrap" + i, { xPercent: scroolSpeed(-40, true) });
    }
  }
...
function moveLogoPattern2() {
  ...
  e.to(".we-do_section .call-bg-logo-wrap", { xPercent: scroolSpeed(28), transformOrigin: "right center" });
}
...
function moveLogoEmeil1() {
  ...
  e.to(".services_email-wrapper .we-do-email-item_wrap", { xPercent: scroolSpeed(-40, true) });
}
...
function moveLogoEmeil() {
  ...
  e.to(".we-do_email-wrapper .we-do-email-item_wrap", { xPercent: scroolSpeed(-40, true) });
}
